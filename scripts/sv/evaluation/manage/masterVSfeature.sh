...
parallel_grep 1 "masterOnly.ctgNamesWithMatchingPacBio.txt" "$GATKVCF_m" | \
    ...
    > temp.FN.bed
sed -e $'s/,/\t/g' temp.FN.bed | gsort -V > suspectsFN.bed

## FP
parallel_grep 1 "featureOnly.ctgNamesNOMatchingPacBio.txt" temp.feature.var.txt | \
    awk 'match($8, /END=[0-9]+/){ending=substr($8,RSTART+4,RLENGTH-4); print $1","$2","ending","$3}{print $1","$2","$2","$3}' \
    > temp.FP.bed
sed -e $'s/,/\t/g' temp.FP.bed | gsort -V > suspectsFP.bed
